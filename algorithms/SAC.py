...
class SAC(tf.keras.layers.Layer):
    ...
    def __init__(self, env, ent_coef='auto', seed=0, hidden_dim=100):
    ...
    def learn(self, vec_env, total_timesteps, log_interval, checkpoint_interval, seed, result_filepath, checkpoint_path, result=None, verbose=1):
        ...
        obs = self.env.reset()
        for iteration in tqdm(range(total_timesteps + 1), desc='SAC', ncols=70, mininterval=1):
            # Take an action
    ...
    @staticmethod
    def load(filepath, env, seed, hidden_dim):
        with open(filepath, 'rb') as f:
            parameters = pickle.load(f)

        model = SAC(env, seed=seed, hidden_dim=hidden_dim)
